...
export default function FireList() {
  ...
  async function getAndSendLocation() {
    ...
    try {
      const res = await axios.patch(`${API_URL}/officers/me/location`, {
        latitude: loc.coords.latitude,
        longitude: loc.coords.longitude,
      }, { withCredentials: true })
      setLastSentAt(res.data.last_updated)
      setSendStatus('sent')
    } catch (e: any) {
      setSendError(e?.response?.data?.detail ?? e?.message ?? 'Failed to send location')
      setSendStatus('error')
    }
  }
